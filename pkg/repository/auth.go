...
//func (auth AuthRepository) CreateUser(user *user.Resource) (int64, error) {
//	var id int64
//	query := fmt.Sprintf("INSERT INTO %s (email, password_hash, created_at) VALUES ($1, $2, $3) RETURNING id", user)
...
//	var id int64
//	query := fmt.Sprintf("SELECT id FROM %s WHERE email = $1 AND password_hash = $2 LIMIT 1", user.UserTableName)
//	ctx, cancel := context.WithTimeout(context.Background(), timeOutQuery)
//	defer cancel()
//
